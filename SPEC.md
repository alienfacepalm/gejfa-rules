...
- **Sticky search row**: autofocus-free large input, clear ×, mic (Web Speech API where supported), and the ☰ category filter button. **Reset to home** is the logo tap on the hero (`resetAll()`); a dedicated always-visible home button (`#resetHomeBtn`) existed briefly but was removed as redundant — every sub-state already has its own visible escape (clear × for search, "← All topics" for an open card, the category sheet's "All topics" for the filter), so the home button never earned its slot in the search row.
