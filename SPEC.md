...
- **Deep-linkable filters and cards**: the level, category, search text, and the currently open card are all reflected in the URL (`?q=&level=&cat=&doc=`) via `history.replaceState` (no history spam while typing), so a coach can share or bookmark the exact view they're looking at — including one specific rule or situation (`?doc=rule:II-9` / `?doc=sit:S-onside`). On load, `?doc=` takes over the view entirely (opens that one card, skips the generic list render); otherwise URL params win over the saved level default, and an explicit `?level=` in a shared link also becomes the new saved default (opening a team's link implies that's their level going forward).
