...
- **Install prompt**: one-time banner on first visit, with copy matched to the detected device — **Add to iPhone / Add to iPad** (iPadOS 13+ is detected via Macintosh UA + touch points), **Add to phone** (Android), or **Install app** (desktop) — or **Later**. Install action uses the native `beforeinstallprompt` where available (Chrome/Edge); otherwise a step-by-step sheet with device-specific instructions: Share → Add to Home Screen on iOS, browser menu → Install app on Android, address-bar install icon / File → Add to Dock on desktop. Either choice is remembered (localStorage `gejfa-install-v1`); a quiet footer link remains for deciding later. Never shown in standalone mode; hidden permanently after `appinstalled`.
